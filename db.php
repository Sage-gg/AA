<?php
// =============================================================================
// MERGED DATABASE CONNECTION FILE - FIXED REDECLARATION ISSUES
...
// Legacy variables for backward compatibility - only set if not already defined
if (!isset($host)) {
    $host = DB_HOST;
    $username = DB_USER;
    $password = DB_PASS;
...
if (!function_exists('cleanupUserSession')) {
...
if (!function_exists('isIPLockedOut')) {
    function isIPLockedOut() {
        ...
        if (!$success_stmt) {
            return false; // If can't prepare statement, don't lock out
        }
...
if (!function_exists('getLockoutTimeRemaining')) {
...
if (!function_exists('cleanOldLoginAttempts')) {
...
if (!function_exists('debugLockoutStatus')) {
...
FIXES IMPLEMENTED:
- Added DB_CONNECTION_LOADED constant to prevent multiple inclusions
- Added function_exists() checks for ALL functions
- Added checks for constant definitions (defined())
- Added checks for variable initialization (isset())
- Added null checks and error handling for database statements
- Added singleton pattern for getDBConnection() to prevent multiple connections
- Added connection ping checks to ensure connections are alive
...
- Constants and variables are only defined if not already set
...
- Complete redeclaration prevention system
*/
